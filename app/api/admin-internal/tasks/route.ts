...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ tasks: data || [], page, pageSize })
  } catch (e:any) {
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const supa = createServiceClient()
    await supa.from('tasks').delete().in('id', ids)

    await adminAuditLog(auth, 'tasks.bulk_delete', { count: ids.length })
