    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto p-6">
        ...
        <header className="bg-white shadow-sm border-b mb-6">
          <div className="max-w-4xl mx-auto px-4 py-4">
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-semibold text-gray-900">{t('team.title') || 'Team'}</h1>
        ...
        <form onSubmit={addMember} className="flex gap-3 bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6">
          <input value={email} onChange={(e)=>setEmail(e.target.value)} type="email" required placeholder={t('team.invitePlaceholder') || 'Invite by email'} className="flex-1 px-3 py-2 rounded-lg bg-white border border-gray-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-200" />
          <select value={role} onChange={(e)=>setRole(e.target.value)} className="px-3 py-2 rounded-lg bg-white border border-gray-300 focus:border-purple-500 focus:ring-2 focus:ring-purple-200">
            <option value="owner">{t('team.role.owner') || 'Owner'}</option>
            <option value="admin">{t('team.role.admin') || 'Admin'}</option>
            <option value="member">{t('team.role.member') || 'Member'}</option>
          </select>
          <button className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg flex items-center gap-2"><UserPlus className="w-4 h-4" /> {t('team.invite') || 'Invite'}</button>
        </form>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 divide-y divide-gray-200">
          {(team || []).map((m:any) => (
            <div key={m.id} className="flex items-center justify-between p-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={async () => { await fetch(`/api/team/${m.id}`, { method: 'DELETE' }); mutate() }} className="p-2 hover:bg-red-50 rounded-lg border border-gray-200 text-red-600">
          ...
          {(!team || team.length===0) && (
            <div className="text-center p-8 text-gray-600">{t('team.empty') || 'No team members yet.'}</div>
          )}
        </div>
      </div>
    </div>
